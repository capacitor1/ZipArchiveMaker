...
namespace ZipArchiveMaker.src
{
    internal class TwoLayerZip1
    {
        ...
        {
            var f = new ZipEntry(entrypath + ".zst");
            f.CompressionMethod = CompressionMethod.Stored;
            zipStream.PutNextEntry(f);
            await SingleZstd1.CreateZst(item,"_Temp",level);
            using (var fs = File.OpenRead("_Temp"))
            {
